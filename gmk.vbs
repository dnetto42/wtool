...
' Set OutPutFile = FSO.OpenTextFile(dpath & "\" & CSName & ".txt" ,8 , True)
...
OutPutFile.WriteLine " ......╙╩╠╠╠╠╠╠╠╠╠╠╠╠╬╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╝╜......"
OutPutFile.WriteLine " ..........`╙╚╩╬╠╠╠╠╠╠╩╝╜╙`╙╙╝╬╠╠╠╠╠╠╠╩╝╙`.........."
OutPutFile.WriteLine " .......................................by.dnetto..."
    ...
    OutPutFile.WriteLine "     ------------------------------------------"
    OutPutFile.WriteLine ""
    OutPutFile.WriteLine "     Computer Name:        " & CSName
    OutPutFile.WriteLine ""
    OutPutFile.WriteLine ""
    OutPutFile.WriteLine "     Operating System Information:"
    OutPutFile.WriteLine "     ============================="
    OutPutFile.WriteLine "     Operating System:     " & Caption & Arch
    OutPutFile.WriteLine "     Version:               " & Version & " " & Sku & " " & CSDVersion
    OutPutFile.WriteLine "     Build Type:            " & BuildType
    OutPutFile.WriteLine "     Locale:                " & Locale
    OutPutFile.WriteLine "     Serial Number:         " & SerialNumber
    OutPutFile.WriteLine ""
    OutPutFile.WriteLine "     Current Time Zone:     " & TZName
    OutPutFile.WriteLine "     Offset from UTC:       " & CurrentTimeZone/60 & " hours"
    OutPutFile.WriteLine "     DST In Effect:         " & DaylightInEffect
    OutPutFile.WriteLine ""
'//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
'//
...
GetKeys()
    ...
    OutPutFile.WriteLine ""
    OutPutFile.WriteLine "     Install Date:          " & InstallDate
    OutPutFile.WriteLine "     Last Boot Time:        " & LastBootUpTime
    OutPutFile.WriteLine "     Local Date/Time:       " & Now()
    OutPutFile.WriteLine ""
    OutPutFile.WriteLine "     System Status:         " & Status
    OutPutFile.WriteLine ""
    OutPutFile.WriteLine ""
    OutPutFile.WriteLine "     Hardware Information:"
    OutPutFile.WriteLine "     ====================="
...
'// Set and echo variables gathered from Win32_Processor class:
    ...
    For Each objProcItem in colProcItems
        ...
        End Select
    OutPutFile.WriteLine "     CPU:                  " & objProcItem.Name & " (" & CPUArch & ")"
    Next
    OutPutFile.WriteLine ""
    OutPutFile.WriteLine "     Physical Memory:      " & TotalMemory & " GB"
    OutPutFile.WriteLine ""
...
'// Set variables gathered from Win32_VideoController class:
    ...
    For Each objVideoItem in colVideoItems
        OutPutFile.WriteLine "     Video Card:           " & objVideoItem.Name

        If Not objVideoItem.AdapterDACType = "" Then
            OutPutFile.WriteLine "     Adapter DAC:           " & objVideoItem.AdapterDACType
        End if

        OutPutFile.WriteLine "     PNP Device ID:         " & objVideoItem.PNPDeviceID

        If Not objVideoItem.AdapterRAM = "" Then
            OutPutFile.WriteLine "     Video RAM:             " & objVideoItem.AdapterRAM/1024^2 & " MB"
        End If

        OutPutFile.WriteLine "     Driver Version:        " & objVideoItem.DriverVersion

        objSWbemDateTime.Value = objVideoItem.DriverDate
        DriverDate = objSWbemDateTime.GetVarDate(False)
        OutPutFile.WriteLine "     Driver Date:           " & DriverDate
...
'// Set variables gathered from Win32_SoundDevice class:
    ...
    For Each objSoundItem in colSoundItems
        OutPutFile.WriteLine "     Sound Card:           " & objSoundItem.Name
        OutPutFile.WriteLine "     Manufacturer:          " & objSoundItem.Manufacturer
        OutPutFile.WriteLine "     PNP Device ID:         " & objSoundItem.PNPDeviceID
        OutPutFile.WriteLine ""
    Next
...
'// Set variables gathered from Win32_LogicalDisk class:
    ...
    For Each objDiskItem in colDiskItems
        If objDiskItem.DriveType = 3 Then
            OutPutFile.WriteLine "     Volume:               " & objDiskItem.Caption
            OutPutFile.WriteLine "     Compressed:            " & objDiskItem.Compressed
            OutPutFile.WriteLine "     File System:           " & objDiskItem.FileSystem
            OutPutFile.WriteLine "     Volume Size:           " & FormatNumber(objDiskItem.Size/1024^3, 2) & " GB"
            OutPutFile.WriteLine "     Free Space:            " & FormatNumber(objDiskItem.FreeSpace/1024^3, 2) & " GB"
            OutPutFile.WriteLine ""
        End If
    Next
...
'// Set variables gathered from Win32_NetworkAdapterConfiguration class:
    ...
    For Each objNetAdapConfigItem in colNetAdapConfigItems
        ...
        Else
            OutPutFile.WriteLine "     Network Adapter:      " & objNetAdapConfigItem.Description
            OutPutFile.WriteLine "     MAC Address:           " & objNetAdapConfigItem.MACAddress
            OutPutFile.WriteLine "     DHCP Enabled:          " & objNetAdapConfigItem.DHCPEnabled
            OutPutFile.WriteLine "     IP Address:            " & Join(objNetAdapConfigItem.IPAddress, ",")
            OutPutFile.WriteLine "     Subnet Mask:           " & Join(objNetAdapConfigItem.IPSubnet, ",")
           ' OutPutFile.WriteLine "     Default Gateway:       " & Join(objNetAdapConfigItem.DefaultIPGateway, ",")
            If objNetAdapConfigItem.DHCPEnabled = True Then
                ...
                OutPutFile.WriteLine "     Lease Obtained:        " & DHCPLeaseObtained

                objSWbemDateTime.Value = objNetAdapConfigItem.DHCPLeaseExpires
                DHCPLeaseExpires = objSWbemDateTime.GetVarDate(True)
                OutPutFile.WriteLine "     Lease Exipres:         " & DHCPLeaseExpires

                OutPutFile.WriteLine "     DHCP Servers:          " & objNetAdapConfigItem.DHCPServer
            End If
            ' OutPutFile.WriteLine "     DNS Server:            " & Join(objNetAdapConfigItem.DNSServerSearchOrder, ",")
            If Not objNetAdapConfigItem.WINSPrimaryServer = "" Then
                OutPutFile.WriteLine "     WINS Primary Server:   " & objNetAdapConfigItem.WINSPrimaryServer
                If Not objNetAdapConfigItem.WINSSecondaryServer = "" Then
                    OutPutFile.WriteLine "     WINS Secondary Server: " & objNetAdapConfigItem.WINSPrimaryServer
                End If
                OutPutFile.WriteLine "     Enable LMHosts Lookup: " & objNetAdapConfigItem.WINSEnableLMHostsLookup
            End If
            OutPutFile.WriteLine ""
        End If
    Next
    ...
    OutPutFile.WriteLine "     ==================="
    OutPutFile.WriteLine "     Computer:             " & CompSysName
    OutPutFile.WriteLine "     Serial Number:         " & IdentifyingNumber
    OutPutFile.WriteLine "     BIOS Version:          " & SMBIOSVersion
    OutPutFile.WriteLine "     UUID:                  " & UUID
    OutPutFile.WriteLine ""
...
End Function
    ...
    Public Function decodeKey(iValues, strProduct)
        ...
        OutPutFile.WriteLine "     " & strProduct & "" & foundKeys(strKey)
    End Function
...
Sub PressEnter()
    ...
    OutPutFile.StdOut.Write strMessage

    Do While Not OutPutFile.StdIn.AtEndOfLine
        Input = OutPutFile.StdIn.Read(1)
    Loop
End Sub
...
Sub ElevateThisScript()
	...
	scriptEngine = Ucase(Mid(OutPutFile.FullName,InstrRev(OutPutFile.FullName,"\")+1))
	engineFolder = Left(OutPutFile.FullName,InstrRev(OutPutFile.FullName,"\"))
	argString = ""

	Set Args = OutPutFile.Arguments
	...
	Else
		...
		objShellApp.ShellExecute scriptCommand, " """ & OutPutFile.ScriptFullName & """" & argString, "", "runas"
		OutPutFile.Quit
	End If
End Sub

MsgBox "files has been created",vbExclamation , "by dnetto"
Set FSO= Nothing
...
' Set WshShell = CreateObject("OutPutFile.Shell")
' keystr = ConvertToKey(WshShell.RegRead("HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\DigitalProductId"))
' dpath = CreateObject("Scripting.FileSystemObject").GetParentFolderName(OutPutFile.ScriptFullName)
' MsgBox(keystr)
'
' Title = "save in file ?"
...
'
' pcDetails = "--- Computer Properties ---"
' pcDetails = pcDetails & "Name: " & objComp.name
' pcDetails = pcDetails & "cn: " & objComp.cn
' pcDetails = pcDetails & "DNSHostName: " & objComp.dNSHostName
' pcDetails = pcDetails & "===================="
' pcDetails = pcDetails & "OperatingSystem: " & objComp.operatingSystem
' pcDetails = pcDetails & "ServicePack: " & objComp.operatingSystemServicePack
' pcDetails = pcDetails & "======================="
' pcDetails = pcDetails & "SAMAccountName: " & objComp.sAMAccountName
' pcDetails = pcDetails & "Location: " & objComp.location
' pcDetails = pcDetails & "UserAccountControl: " & objComp.userAccountControl
' pcDetails = pcDetails & "PrimaryGroupID: " & objComp.primaryGroupID
' pcDetails = pcDetails & "WhenCreated: " & objComp.whenCreated
' pcDetails = pcDetails & "WhenChanged: " & objComp.whenChanged
' pcDetails = pcDetails & "DistinguishedName: " & objComp.distinguishedName
'
' pcStts = pcStts & pcDetails